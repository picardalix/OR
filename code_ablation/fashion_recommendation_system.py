...
class EmbeddingGenerator:
    """
    Encodeur image→embedding avec plusieurs backends compatibles.
    Choisis le backend via model_name:
      - "patrickjohncyh/fashion-clip" (par défaut)
      - "openclip:ViT-B-32/laion2b_s34b_b79k"
      - "timm:vit_base_patch16_224"
      - "timm:convnext_base"
      - "timm:beit_base_patch16_224"
    """
    def __init__(self, model_name: str = "patrickjohncyh/fashion-clip", dtype: str = "float32"):
        ...
        if model_name.startswith("openclip:"):
            try:
                ...
                raise RuntimeError("open-clip-torch n'est pas installé. `pip install open-clip-torch`") from e
            # ex: "openclip:ViT-B-32/laion2b_s34b_b79k"
            spec = model_name.split(":", 1)[1]
            if "/" in spec:
                arch, ckpt = spec.split("/", 1)
            else:
                arch, ckpt = spec, "laion2b_s34b_b79k"
            self._kind = "openclip"
            ...
            self._model = timm.create_model(timm_name, pretrained=True).to(self.device).eval()
            cfg = resolve_data_config({}, model=self._model)
            self._preprocess = create_transform(**cfg)
            try:
                self._dim = int(self._model.num_features)
            except Exception:
                self._dim = 768  # fallback standard ViT-B

        else:
            # FashionCLIP par défaut
            try:
                from fashion_clip.fashion_clip import FashionCLIP
            except Exception as e:
                raise RuntimeError("FashionCLIP introuvable. Installe `fashion-clip`.") from e
            self._kind = "fclip"
            self._model = FashionCLIP(model_name)
            self._dim = 512  # FashionCLIP retourne 512 dims

    @property
    def dim(self) -> int:
        return int(self._dim) if self._dim else 512

    # --- utils ---
    def _sanitize(self, emb: np.ndarray) -> np.ndarray:
        if emb is None:
    ...
    def encode_images(self, paths: List[str], batch_size: int = 16) -> np.ndarray:
        ...
        if self._kind == "fclip":
            embs = self._model.encode_images(paths, batch_size=batch_size)  # type: ignore
            embs = np.asarray(embs, dtype=np.float32)
            embs = np.vstack([self._sanitize(e) for e in embs])
            return embs
        ...
        for p in paths:
            try:
                ...
                t = self._preprocess(img)  # type: ignore
            except Exception:
                # image cassée -> tenseur zéro
                t = _torch.zeros(3, 224, 224)
            xs.append(t)
        x = _torch.stack(xs).to(self.device)

        with _torch.inference_mode():
            if self._kind == "openclip":
                feats = self._model.encode_image(x)  # type: ignore
            else:
                # timm
                try:
                    feats = self._model.forward_features(x)  # type: ignore
                    if isinstance(feats, (list, tuple)):
                        feats = feats[-1]
                    if feats.ndim == 4:
                        feats = feats.mean(dim=(2, 3))  # GAP
                except Exception:
                    feats = self._model(x)  # type: ignore
            feats = feats.float().cpu().numpy()

        feats = np.vstack([self._sanitize(f) for f in feats])
        return feats

    # ---------- utils cache ----------
    def _fingerprint(self, path: str) -> str:
        """Empreinte rapide: chemin + mtime_ns + taille (évite de lire le fichier)."""
        st = os.stat(path)
        base = f"{path}|{st.st_mtime_ns}|{st.st_size}|{self.model_name}|{self._cache_version}"
        return hashlib.sha1(base.encode("utf-8")).hexdigest()

    def _cache_paths(self, key: str) -> Tuple[Optional[Path], Optional[Path]]:
        if not self.cache_dir:
            return None, None
        # .npy pour le vecteur + .json pour méta (facultatif)
        return self.cache_dir / f"{key}.npy", self.cache_dir / f"{key}.json"

    def _cache_get(self, path: str) -> Optional[np.ndarray]:
        key = self._fingerprint(path)

        # mémoire
        if self.mem_cache is not None and key in self.mem_cache:
            return self.mem_cache[key]

        # disque
        vec_path, _ = self._cache_paths(key)
        if vec_path and vec_path.exists():
            try:
                arr = np.load(vec_path)
                # sécurité: on renormalise (au cas où le fichier provient d’une ancienne version)
    ...
    def _cache_put(self, path: str, emb: np.ndarray) -> None:
        key = self._fingerprint(path)
        ...
        # mémoire
        if self.mem_cache is not None:
            self.mem_cache[key] = emb

        # disque (écriture atomique)
        vec_path, meta_path = self._cache_paths(key)
        if vec_path:
            try:
                with tempfile.NamedTemporaryFile(dir=str(vec_path.parent), delete=False) as tmp:
                    np.save(tmp.name, emb)
                    tmp.flush()
                    os.fsync(tmp.fileno())
                    tmp_path = tmp.name
    ...
    def generate_embeddings(self, image_paths: List[str], batch_size: int = 16) -> List[Optional[np.ndarray]]:
        ...
        # 2) encoder uniquement les manquants
        for i in range(0, len(to_encode_paths), batch_size):
            batch_paths = to_encode_paths[i:i + batch_size]
            outs = self._encode_safe(batch_paths, batch_size=min(len(batch_paths), batch_size))
            for local_j, emb in enumerate(outs):
                global_idx = to_encode_indices[i + local_j]
                results[global_idx] = emb
                # 3) écrire au cache si valide
                if emb is not None:
                    self._cache_put(image_paths[global_idx], emb)
